...
Java_YAO_GreenLife_core_NanoDet_init(JNIEnv *env, jclass, jobject assetManager, jboolean useGPU) {
    if (NanoDet::detector == nullptr) {
        AAssetManager *mgr = AAssetManager_fromJava(env, assetManager);
        NanoDet::detector = new NanoDet(mgr, "nanodet-plus-m_416.param", "nanodet-plus-m_416.bin",
                                        useGPU);
    }
}

extern "C" JNIEXPORT jobjectArray JNICALL
Java_YAO_GreenLife_core_NanoDet_detect(JNIEnv *env, jclass, jobject image, jdouble threshold,
                                       jdouble nms_threshold) {
    auto result = NanoDet::detector->detect(env, image, threshold, nms_threshold);
    double start_time = ncnn::get_current_time();

    auto box_cls = env->FindClass("YAO/GreenLife/core/Box");
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "NanoDet", "%.2fms   detect", elasped);
